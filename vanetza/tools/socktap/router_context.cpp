...
RouterContext::RouterContext(const geonet::MIB& mib, TimeTrigger& trigger, vanetza::PositionProvider& positioning, vanetza::security::SecurityEntity* security_entity, bool ignore_own_messages_, bool ignore_rsu_messages_) :
    mib_(mib), router_(trigger.runtime(), mib_),
    trigger_(trigger), positioning_(positioning),
    ignore_own_messages(ignore_own_messages_), ignore_rsu_messages(ignore_rsu_messages_)
{
...
{
    if ((!ignore_own_messages || hdr.source != mib_.itsGnLocalGnAddr.mid()) && (!ignore_rsu_messages || hdr.source.octets[3] != 1) && hdr.type == access::ethertype::GeoNetworking) {
